...
// Git-derived content dates, regenerated by scripts/gen-route-lastmod.mjs.
// Committed as data because Vercel's shallow clone makes build-time git
// history unreliable.
const routeLastmod = routeLastmodJson as Record<string, string>;
...
export default function sitemap(): MetadataRoute.Sitemap {
  const routes: {
    ...
    // Blog entries carry their own content date; static routes fall back
    // to the git-derived date in route-lastmod.json at map time below.
    lastModified?: Date;
  }[] = [
    // Homepage
  ...
  for (const member of teamMembers) {
    routes.push({
      ...
      ...(routeLastmod["__team__"]
        ? { lastModified: new Date(routeLastmod["__team__"]) }
        : {}),
    });
  }
  ...
  return routes.map((route) => {
    const lastModified =
      route.lastModified ??
      (routeLastmod[route.path]
        ? new Date(routeLastmod[route.path])
        : undefined);
    return {
