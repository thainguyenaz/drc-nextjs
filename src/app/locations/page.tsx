  ...
  return (
    <>
      ...
      {locations.map((loc, i) => {
        ...
        return (
          <AnimatedSection key={loc.name} animation={imageLeft ? "fadeLeft" : "fadeRight"}>
            <section className="relative min-h-[500px] flex flex-col lg:flex-row overflow-hidden">
              ...
              <div className={`w-full lg:w-2/5 bg-[#1a2e1a] p-8 md:p-12 lg:p-16 flex flex-col justify-center ${imageLeft ? "" : "lg:order-1"}`}>
                <span className="text-gold font-body text-xs tracking-[0.3em] uppercase font-medium">{loc.label}</span>
                <div className="w-10 h-0.5 bg-gold mt-4 mb-6" />
                <h2 className="font-display text-3xl md:text-4xl text-white font-semibold mb-3">{loc.name}</h2>
                <p className="text-white/50 text-sm mb-4">{loc.address}</p>
                <p className="text-white/70 leading-relaxed mb-4">{loc.description}</p>
                {loc.sqft && <p className="text-gold font-semibold text-sm mb-4">{loc.sqft} facility</p>}
                <div className="flex flex-wrap gap-2 mb-8">
                  {loc.amenities.map((a) => (
                    <span key={a} className="text-xs text-white/70 border border-white/20 rounded-full px-3 py-1.5">{a}</span>
                  ))}
                </div>
                <div className="flex flex-wrap gap-3">
                  <Link href={loc.href} className="bg-gold hover:bg-gold-dark text-white font-semibold text-sm px-6 py-3 rounded-xl transition-colors">Explore Facility</Link>
                  <a href={`tel:${loc.phoneTel}`} className="border border-white/30 hover:border-white/60 text-white font-semibold text-sm px-6 py-3 rounded-xl transition-colors">{loc.phone}</a>
                </div>
              </div>
            </section>
          </AnimatedSection>
        ...
        <div className="max-w-7xl mx-auto">
          <AnimatedSection>
            <div className="text-center mb-12">
              <span className="text-sage font-body text-sm tracking-[0.2em] uppercase font-medium">Virtual Tours</span>
              <div className="w-[60px] h-0.5 bg-gold mx-auto mt-4 mb-4" />
              <h2 className="font-display text-3xl md:text-4xl text-forest font-semibold mb-3">Explore Our Recovery Centers</h2>
              <p className="text-gray-500 max-w-lg mx-auto">Tour our luxury treatment facilities from anywhere in the world.</p>
            </div>
          </AnimatedSection>
            ...
            {locations.map((loc) => (
              <div key={loc.name} className="bg-white rounded-2xl overflow-hidden shadow-lg">
                ...
                <div className="p-6">
                  ...
                  <a href={`tel:${loc.phoneTel}`} className="inline-block bg-gold hover:bg-gold-dark text-white font-semibold text-sm px-5 py-2.5 rounded-xl transition-colors">Schedule a Tour</a>
                </div>
              </div>
            ))}
          </AnimatedGrid>
      ...
      <section className="py-16 md:py-24 bg-white">
        <div className="max-w-container mx-auto px-6">
          <AnimatedSection>
            <div className="text-center mb-12">
              <span className="text-sage font-body text-sm tracking-[0.2em] uppercase font-medium">Programs by Location</span>
              <div className="w-[60px] h-0.5 bg-gold mx-auto mt-4 mb-4" />
              <h2 className="font-display text-3xl md:text-4xl text-forest font-semibold mb-3">Find the Right Level of Care</h2>
              <p className="text-gray-500 max-w-lg mx-auto">Each location offers different programs. Find the one that matches your needs.</p>
            </div>
          </AnimatedSection>
              ...
              <table className="w-full border-collapse">
                <thead>
                  <tr className="bg-[#1a2e1a]">
                    <th className="text-left text-white font-display font-semibold text-sm px-6 py-4 rounded-tl-xl">Program</th>
                    <th className="text-center text-white font-display font-semibold text-sm px-6 py-4">Glendale</th>
                    <th className="text-center text-white font-display font-semibold text-sm px-6 py-4">Scottsdale</th>
                    <th className="text-center text-white font-display font-semibold text-sm px-6 py-4 rounded-tr-xl">Phoenix PHP / IOP</th>
                  </tr>
                </thead>
                <tbody>
                  {programMatrix.map((row, i) => (
                    <tr key={row.program} className={i % 2 === 0 ? "bg-cream/50" : "bg-white"}>
                      ...
                      <td className="text-center px-6 py-4">{row.glendale ? <span className="text-gold text-lg font-bold">&#10003;</span> : <span className="text-gray-300">—</span>}</td>
                      <td className="text-center px-6 py-4">{row.scottsdale ? <span className="text-gold text-lg font-bold">&#10003;</span> : <span className="text-gray-300">—</span>}</td>
                      <td className="text-center px-6 py-4">{row.phoenix ? <span className="text-gold text-lg font-bold">&#10003;</span> : <span className="text-gray-300">—</span>}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
