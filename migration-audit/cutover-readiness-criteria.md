...
1. **All audit phases (Sections A through I) close as PASS or WARN.** Zero CRITICAL findings outstanding.
   - NOT-GREEN 2026-05-28. **CRITICAL**: sitewide FAQ data-integrity blocker with compliance edge, gated on clinical sign-off (24-page dual-source pattern, 7 pages flip-blocking, 1 compliance-flagged). See Threads B.1-B.4 below.
     ...
     - Sub-task B EXPANDED 2026-05-28: original framing (THIN-NX content gap on 3 pages, 25-36% body-text deficit chrome-stripped) was confirmed real, but the 2026-05-28 investigation showed it was one symptom of a deeper sitewide pattern. Four threads now tracked under sub-task B:
       - **Thread B.1 — sitewide `<main>` landmark missing.** CLOSED 2026-05-28 (commit `817019c`). Root `src/app/layout.tsx` now wraps `{children}` in `<main>`; the two files that emitted their own inner `<main>` converted to `<div>` with classes preserved. Verified locally and post-deploy on 5 routes.
       - **Thread B.2 — dual-source FAQ pattern, 24 pages.** Each affected page mounts both `<ConditionFAQ items={faqs}>` (local) and `<FAQSection faqs={faqData[key]}>` (canonical) and renders two visible "Frequently Asked Questions" H2 blocks. 1 of 24 closed (`/addiction/alcoholism-treatment`, commit `5e995b6`); 23 remain. Engineering remediation post-clinical-confirmation will establish `faqData` as single source of truth and remove every local `const faqs` sitewide so the pattern cannot recur.
       - **Thread B.3 — FLIP-BLOCKING: 7 pages have verified factual contradictions inside the dual-source pattern.** YMYL clinical contradictions on length-of-stay (cocaine, PTSD, marijuana, anxiety, prescription drug page durations), claimed-treated condition lists (anxiety disorders, depression types), Phoenix PHP/IOP program cadence, and one **compliance-flagged** item: `/addiction/prescription-drug-addiction-treatment` claims in-house tapering that contradicts the documented no-in-house-detox operating model. Awaiting clinical confirmation via `migration-audit/faq-clinical-confirmation-worksheet.md` (commit `52e85cf`). After Dr. An Nguyen returns the worksheet, engineering applies per-page fixes followed by the Thread B.2 sitewide refactor.
       - **Thread B.4 — reframed.** `T-mental-health-rebuild` is replaced by `T-mental-health-hub-polish` (3 targeted fixes: PTSD card rename, Trauma Therapy card add pointing to `/treatments/trauma-therapy`, and the ConditionsGrid h2/eyebrow change; see `migration-audit/tickets/`). The underlying 27% body-text deficit (691 to 507 words per the May 7 audit) is deprioritized per Thai's 2026-05-31 decision: hub-page-not-parity-target, matching the `/get-help` precedent. Criterion 1 no longer blocks on B.4. The polish ticket ships pre-cutover; the deficit is accepted as non-blocking. The related Cluster A trauma-page canonical decision (Option B) is tracked in `migration-audit/tickets/T-trauma-cluster-canonical-decision.md`.
